...
export class DashboardService {
  ...
  getTotalDashboardData() {
    return this.http
      ...
      .pipe(tap((response) => console.log('Dashboard data fetched', response)));
  }

  getDashboardData(): Observable<any> {
    return this.http.get<any>(`${baseUrl}/dashboard`).pipe(
      catchError((error) => {
    ...
    getFiberCutByState(): Observable<any> {
    return this.http.get<any>(`${baseUrl}/dashboard/top-fiber-cut-states`).pipe(
      catchError((error) => {
        console.error('Error fetching fiber cut data', error);
        return of(this.getMockFiberCutData());
      })
    );
  }

  getFilterOptions(): Observable<any> {
    return this.http.get<any>(`${baseUrl}/dashboard/filter-options`).pipe(
      catchError((error) => {
  ...
  getTicketsByFiberLength() {
    return this.http
      .get<any>(`${baseUrl}/dashboard/top-fiber-cut-states`)
      .pipe(tap((response) => console.log('datos cortes de fibra por estado', response)));
  }

    getMockFiberCutData(): any {
    return {
      topStates: [
        { state: "anzoátegui", fiberCutsCount: 2 },
        { state: "aragua", fiberCutsCount: 1 },
        { state: "falcón", fiberCutsCount: 1 }
      ],
      totalFiberCutTickets: 4
    };
  }
